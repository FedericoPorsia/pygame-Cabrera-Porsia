...
=======
    ...
    print(producto)
>>>>>>> 453b55b6c575ddda97a4250a368fecfb0ea6dacb
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if PLAY_BUTTON.chequearEntrada(MENU_MOUSE_POSICION):
                    instrucciones()
                    corriendo = False
                if OPTIONS_BUTTON.chequearEntrada(MENU_MOUSE_POSICION):
                    instrucciones2()
                    corriendo = False
...
def dibujar_nave2(rect):
    screen.blit(NAVE_ESPACIAL, (rect.x, rect.y))

def manejar_movimiento_nave2(tecla_presionada , rectangulo):
    if tecla_presionada[pygame.K_t] and rectangulo.y - VELOCIDAD_NAVE >= 100 and rectangulo.y - VELOCIDAD_NAVE <= 550: # Arriba
        rectangulo.y -= VELOCIDAD_NAVE

    if tecla_presionada[pygame.K_g] and rectangulo.y - VELOCIDAD_NAVE >= 0 and rectangulo.y - VELOCIDAD_NAVE <= 500: # Abajo
        rectangulo.y += VELOCIDAD_NAVE
def main2():
    rect = pygame.Rect(400, 300, NAVE_ESPACIAL_WIDTH,NAVE_ESPACIAL_HEIGHT)
    rect1 = pygame.Rect(0, 300, NAVE_ESPACIAL_WIDTH,NAVE_ESPACIAL_HEIGHT)
    lista_rectangulos_prod = dibujar_rect_productos()
    diccionario = {}
    contador = 0
    for i in lista_rectangulos_prod:
    ...
    index = index_producto_elegido(productos_en_pantalla, producto_elegido_programa)
    print(productos_en_pantalla)
    dibujar1(screen, productos_en_pantalla, producto,
            producto_candidato, puntos, segundos, nivel)
    time_delay = 1000
    timer_event = pygame.USEREVENT+1
    pygame.time.set_timer(timer_event, time_delay)
    ...
    while corriendo:
        ...
        tecla_presionada = pygame.key.get_pressed()
        manejar_movimiento_nave(tecla_presionada, rect)
        manejar_movimiento_nave2(tecla_presionada, rect1)
...
def instrucciones2():
    ...
    while True:
        ...
        archivo_instrucciones = open("./instrucciones2.txt")
        x = 30
